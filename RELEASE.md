...
```mermaid
flowchart LR
    prep[Prepare Repository] --> changelog[Changelog Updates]
    changelog --> schema[Schema Validation]
    schema --> tag[Tag Release]
    tag --> assets[Asset Attachment]
    assets --> publish[Publish GitHub Release]
```

The numbered sections below reference these nodes to keep the checklist aligned with the visual sequence.

## 1. Prepare the Repository
_Flowchart reference: `Prepare Repository` → `Changelog Updates` → `Schema Validation`._
...
## 2. Tag the Release
_Flowchart reference: `Tag Release` node following successful schema validation._
...
_Flowchart reference: `Asset Attachment` checkpoint leading into `Publish GitHub Release`._
