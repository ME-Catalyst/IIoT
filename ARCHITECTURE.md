...
## 1. System overview
The Node-RED flow provides dual-ingestion (HTTP polling + MQTT subscribe) to capture IO-Link gateway telemetry, normalize it with shared configuration, and persist it into dedicated InfluxDB buckets for visualization in Grafana. The design favors edge autonomy—Node-RED runs near the gateways, buffering structured logs locally so operators can recover context without cloud connectivity.
...
| Visualization | Grafana dashboards under `docs/grafana/` | Dashboards query Flux from the three buckets. |
...
- JSON Schemas under `docs/schemas/` guard config changes; run `ajv-cli` via the commands in `README.md` to validate.
...
| Grafana | Visualization + alerting | Import dashboards from `docs/grafana/`; configure data source pointing at InfluxDB organization and token. |
...
- MQTT pipeline scales with topic volume; ensure Node-RED instance has sufficient memory for bursty telemetry.
...
- Operational lifecycle diagram: [`docs/pipeline-operations.svg`](docs/pipeline-operations.svg)
